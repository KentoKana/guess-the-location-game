...
class App extends Component {
  ...
  getCoordWithStreetView = latLng => {
    axios
      .get(
        `https://maps.googleapis.com/maps/api/streetview/metadata?key=${process.env.REACT_APP_GOOGLE_MAPS_API_KEY}&location=${latLng.lat},${latLng.lon}`
      )
      .then(res => {
        // console.log(latLng);
  ...
  getCountries = () => {
    axios
      .get(`https://cors-anywhere.herokuapp.com/http://country.io/capital.json`)
      .then(res => {
  ...
  getCoord = () => {
    ...
    geocoder
      .search({ q: this.state.countries[this.getRandomCity()] })
      .then(response => {
